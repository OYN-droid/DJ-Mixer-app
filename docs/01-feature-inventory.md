...
| Crate and music library | Partially Working | `#sourceForm`, `#sourceDrop`, `#sourceList`; crate functions | Local files work in memory. URL metadata persists. Direct audio fetch depends on CORS | Local files vanish on refresh. Provider links are not streams. High |
